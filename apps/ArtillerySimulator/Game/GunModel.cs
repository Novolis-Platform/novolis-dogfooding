...
internal sealed class GunModel
{
  ...
  private static readonly float[] ChargeSpeeds = [400f, 600f, 800f];
  ...
  public float MuzzleSpeedMps => ChargeSpeeds[ChargeIndex];
  ...
  public void NudgeElevation(float delta) => ElevationDegrees = Math.Clamp(ElevationDegrees + delta, 0f, 70f);

  public void NudgeAzimuth(float delta) => AzimuthDegrees = (AzimuthDegrees + delta + 360f) % 360f;

  public void SetCharge(int index) => ChargeIndex = Math.Clamp(index, 0, ChargeSpeeds.Length - 1);
  ...
  public void Draw(RayGameContext ctx, Vector3 gunPivot)
  {
    var dir = BarrelDirection();
    var muzzle = MuzzlePosition(gunPivot);
    var carriage = Color.FromArgb(255, 90, 95, 100);
    var tube = Color.FromArgb(255, 120, 125, 130);
    ctx.DrawBolt(gunPivot, gunPivot + new Vector3(0f, 0.6f, 0f), carriage);
    ctx.DrawBolt(gunPivot + new Vector3(0f, 0.4f, 0f), muzzle, tube);
    ctx.DrawGlowSphereWires(muzzle, MuzzleRadius, Color.FromArgb(255, 160, 170, 180));
  }
}
